...
Além das regras de definição de funções descritas no tópico acima, é necessário que todas elas estejam documentadas seguindo o padrão [**roxygen2**](https://cran.r-project.org/web/packages/roxygen2/vignettes/roxygen2.html), incluindo funções auxiliares. Cada função deve estar em um arquivo separado. Além disso, as exceções devem ser tratadas dentro de blocos `tryCatch()`. 

Para ver um exemplo de função documentada, cheque o [guia de estilo do tidyverse](https://style.tidyverse.org/documentation.html) ou veja o exemplo que inserimos  na pasta `example/functions`. 
...
Para entrega de produtos que envolvam código por parceiros comerciais da re.green, os arquivos deverão ser entregues em um repositório no github da empresa.  O link será fornecido por ela e será dada autorização para inclusão de colaboradores(as) com papel de escrita. O nome do repositório deverá seguir o padrão **partner_\<nome do parceiro comercial>\_product_\<nome do produto>**\. Exemplo: **partner_IIS_product_biodiversity**.
...
A ramificação `main` deve armazenar a base de código principal e nunca deve conter código quebrado. Já o desenvolvimento deve ser encapsulado na ramificação `dev`. Esse ramo funciona como pai para as ramificações de features e hot-fix, servindo como ponto de bifurcação e união para elas. Uma vez que o ramo `dev` tenha adquirido código novo o suficientes para que o produto seja considerado uma nova versão, deve-se fazer o merge com o ramo principal `main`.
...
Todo conjunto de dados deve apresentar um arquivo de metadados com informações que descrevem o conjunto, de acordo com os formatos abaixo:
...
Para mais um exemplo, consulte a pasta **examples/metadata/tabular**.
...
Para ver um exemplo de metadados de um shapefile, consulte a pasta **examples/metadata/shape**. 
**Importante**: Todo shapefile deve ser exportado em formato **gpkg**.
...
Para ver um exemplo de metadados de um raster, consulte a pasta **examples/metadata/raster**. 
